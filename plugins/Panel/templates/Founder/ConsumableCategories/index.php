 ...
 * @var iterable<\Cake\Datasource\EntityInterface> $consumableCategories
 */
$this->assign('title', __d('panel', 'Consumable categories'));

$this->start('breadcrumbs');
$breadcrumbs = [
    ['title' => __d('panel', 'Directory')],
    ['title' => __d('panel', 'Consumable categories')]
];
...
<div class="subheader">
    <h1 class="subheader-title">
        <i class="subheader-icon fal fa-table"></i> <?= __d('panel', 'Consumable categories') ?>
    </h1>
</div>
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    <table class="table table-bordered table-hover table-striped w-100 datatable">
                        ...
                        <tbody>
                            <?php foreach ($consumableCategories as $consumableCategory): ?>
                            <tr>
                                <td><?= h($consumableCategory->title) ?></td>
                                <td><?= $consumableCategory->description ?></td>
                                <td class="text-center">
                                    <?php
                                    echo $this->Html->link(
                                        $this->Html->tag('i', '', ['class' => 'fal fa-pencil']),
                                        ['action' => 'edit', h($consumableCategory->id)],
                                        ['escape' => false]
                                    );
                                    ?>
                                </td>
