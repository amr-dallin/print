<?php
$this->assign('title', __d('panel', 'Cash register receipts'));

$this->start('breadcrumbs');
$breadcrumbs = [
    ['title' => __d('panel', 'Cash register')],
    ['title' => __d('panel', 'Receipts')]
];
echo $this->element('breadcrumbs', ['breadcrumbs' => $breadcrumbs]);
$this->end();

$this->start('navigation');
$menu['cash_register']['receipts'] = true;
...
<div class="subheader">
    <h1 class="subheader-title">
        <i class="subheader-icon fal fa-align-left"></i> <?= __d('panel', 'Cash register receipts') ?>
    </h1>
</div>
...
<div class="row">
    <div class="col-xl-12">
        <div id="panel-1" class="panel" data-panel-close data-panel-sortable data-panel-fullscreen data-panel-refresh data-panel-locked data-panel-collapsed>
            <div class="panel-hdr">
                <h2><?= __d('panel', 'Cash register receipts') ?></h2>
                <button type="button" class="btn btn-xs btn-success mr-3" data-toggle="modal" data-target="#js-op-receipt-create-model"><?= __d('panel', 'Create') ?></button>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        echo $this->Form->control('date_receipted', [
                                            'label' => __d('panel', 'Date receiptd') . $this->Html->tag('span' , '*', ['class' => 'ml-1 text-danger']),
                                            'escape' => false
                                        ]);
                                        ?>
                                    </div>
                    ...
                    <table class="table table-bordered table-hover table-striped w-100 datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="all text-center"><?= __d('panel', 'Date receipted') ?></th>
                            ...
                            <tr>
                                <td class="text-center">
                                    ...
                                    echo $this->Form->postLink(
                                        ...
                                        $this->Url->build(['action' => 'delete', h($opReceipt->id)]),
                                        [
                                            'class' => 'color-danger-900',
                                            'data-title' => __d('panel', 'Are you sure you want to delete the cash register receipt?'),
                                            'data-message' => __d('panel', 'Deletion eliminates the possibility of data recovery.')
                                        ]
                                    );
                                    ?>
                                </td>
                                <td><?= $opReceipt->title ?></td>
                                <td><?= $opReceipt->description ?></td>
                                <td class="text-center" data-order="<?= $opReceipt->date_receipted->format('c') ?>">
                                    <?= $opReceipt->date_receipted->format('d.m.Y') ?>
                                </td>
                                <td class="text-right"><?= $this->Number->currency($opReceipt->amount, 'UZS', ['locale' => 'uz_UZ']) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
